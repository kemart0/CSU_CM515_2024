...
(from 'str()' function 34,786 rows × 13 columns)
```{r}
nrow(surveys)
#Rows: 34,786
ncol(surveys)
...
```{r}
#Q3
nrows_list<- nrow(surveys)
surveys_last<-surveys[nrows_list,]
surveys_last
```
...
```{r}
#Q9 
surveys_hindfeet<- surveys %>% 
  filter(!is.na(hindfoot_length)) %>% 
  mutate(hindfoot_cm=hindfoot_length/10) %>% 
  filter(hindfoot_cm<3) %>% 
  select(species_id, hindfoot_cm) 
head(surveys_hindfeet)
...
#I don't understand why there are two entries for 1978, maybe because they have two animals with the same weight as the max?
...
```{r}
#Q13- this will involve pivoting from long to wide?

wide_surveys<- surveys %>% 
  group_by(plot_id, year) %>% 
  summarize(number_genera=n_distinct(genus)) %>% 
  pivot_wider(names_from = year, values_from = number_genera) %>% 
  print()
...
wide_surveys %>% pivot_longer(names_to = "year", values_to = "number_genera", cols = -plot_id)
